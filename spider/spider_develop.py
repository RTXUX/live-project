...
class MeituanSpider(object):
    ...
    baseUrl = ("http://api.meituan.com/group/v4/deal/select/city/44/cate/1?"
    ...
    def parse(self,url):
        ...
        while True:
            try:
                # https_ = random.choice(https)
                # http_ = random.choice(http)
                a_url = 'http://www.xicidaili.com/nn/'
                a_headers = {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.143 Safari/537.36'
                }
                ip_list = get_ip_list(a_url, headers=a_headers)
